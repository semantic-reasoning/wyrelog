...
static WylEngine *
open_engine_from_synthetic_templates (void)
{
  WylEngine *engine = NULL;
  /* Manifest authenticity is outside these synthetic evaluator fixtures. */
  wyrelog_error_t rc =
      wyl_engine_open_with_options (WYL_TEST_TEMPLATE_DIR, 1, FALSE, &engine);
...
{
  g_autoptr (WylEngine) engine = open_engine_from_synthetic_templates ();
  gint64 row[3];
...
{
  WylEngine *engine = open_engine_from_synthetic_templates ();
