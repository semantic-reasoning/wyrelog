...
/*
 * #1030: clear_strv zeroes each value before freeing it, because the strict
 * JSON parser is how credentials arrive -- a refresh token, and at the
 * service-token route a credential secret.  Zeroing is not observable from
 * outside, so what this pins is the half that can break: a partly populated
 * array.  That is the normal shape on the parser's failure path, where an
 * object that parsed its first field and then failed leaves the later slots
 * NULL, so a wipe that dereferenced every slot would crash on exactly the
 * path it was added to protect.
 */
static void
test_clear_strv_tolerates_unpopulated_slots (void)
{
  ...
  /* A second field that is a number where a string is required: the first
   * slot is populated, the second is not. */
  const gchar *partial = "{\"subject\":\"svc:jobs:worker\",\"tenant\":7}";
  g_assert_false (wyl_daemon_http_dup_strict_json_object (partial,
      strlen (partial), fields, G_N_ELEMENTS (fields), values));
  g_assert_null (values[0]);
  g_assert_null (values[1]);

  /* The same shape built directly, since the parser clears what it frees and
   * so never hands a mixed array back to a caller. */
  values[0] = g_strdup ("svc:jobs:worker");
  values[1] = NULL;
  wyl_daemon_http_clear_strv (values, G_N_ELEMENTS (values));
  g_assert_null (values[0]);
  g_assert_null (values[1]);

  /* Every slot NULL, the shape the parser's early validation produces. */
  wyl_daemon_http_clear_strv (values, G_N_ELEMENTS (values));
  g_assert_null (values[0]);
}
...
main (int argc, char **argv)
{
  ...
  g_test_add_func ("/daemon/http/clear-strv-unpopulated",
      test_clear_strv_tolerates_unpopulated_slots);
