...
check_bootstrap_seed_contract (const gchar *contents, gsize len)
{
  static const gchar *const snippets[] = {
    ...
    ".decl approved_data_plane_permission(name: symbol)",
    ".decl permission_plane(permission: symbol, plane: symbol)",
    ...
    "permission(\"wr.tenant.manage\").",
    "permission(\"wr.audit.read\").",
    "permission(\"wr.audit.write\").",
    "permission(\"wr.service_principal.manage\").",
    "permission(\"wr.service_credential.manage\").",
