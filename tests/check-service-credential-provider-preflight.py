...
"""Redacted state and HTTP proof for a providerless credential handoff."""
import argparse
import base64
...
def snapshot(args):
    uri = Path(args.policy).resolve().as_uri() + "?mode=ro"
...
def check_private_output(args, state, body=b""):
    ...
    needles = [token, key, args.policy.encode(), args.key.encode(),
               ...
               args.publication_root.encode(), b"svc:svc-app", b"admin1",
               b"admin2", b"tenant-a", b"__wr_default", DESTINATION.encode(),
               state["request_id"].encode()]
    logs = []
...
def request(args):
    ...
    status, body = _post(url, token=_read_token(args.token_file),
...
def main():
    ...
    try:
        if args.phase == "request":
            ...
            require(snapshot(args) == state["counts"],
                    "durable credential state changed")
