...
main (void)
{
  ...
  if (wyl_dl_check_functional_ic (NULL, 1, 2, NULL) != WYRELOG_E_INVALID)
    return wyl_test_normalize_exit_status (100);
  ...
  /* Case 104: reject (B) -- (auth, login_ok) -> {mfa_required, active}. */
  {
    ...
    if (g_strcmp0 (w.head, "principal_transition") != 0)
      return wyl_test_normalize_exit_status (1041);
    if (w.key_arity != 2)
      return wyl_test_normalize_exit_status (1042);
    if (g_strcmp0 (w.key[0], "auth") != 0)
      return wyl_test_normalize_exit_status (1043);
    if (g_strcmp0 (w.key[1], "login_ok") != 0)
      return wyl_test_normalize_exit_status (1044);
    if (g_strcmp0 (w.value_a, "mfa_required") != 0)
      return wyl_test_normalize_exit_status (1045);
    if (g_strcmp0 (w.value_b, "active") != 0)
      return wyl_test_normalize_exit_status (1046);
    if (w.first_index != 0)
      return wyl_test_normalize_exit_status (1047);
    if (w.conflict_index != 1)
      return wyl_test_normalize_exit_status (1048);
  }
  ...
  /* Case 105: reject (C) -- mix of valid + non-functional rows. */
  {
    ...
    if (w.first_index != 0 || w.conflict_index != 3)
      return wyl_test_normalize_exit_status (1051);
  }
  ...
  /* Case 108: cross-head same key -> OK (heads differentiate the group). */
  {
    ...
    if (wyl_dl_check_functional_ic (rows, 2, 2, NULL) != WYRELOG_E_OK)
      return wyl_test_normalize_exit_status (108);
  }
  ...
  if (wyl_dl_assert_edb_empty (NULL, 0, "policy_violation",
      NULL) != WYRELOG_E_OK)
    return wyl_test_normalize_exit_status (110);
  ...
  /* Case 111: rows present but none match expected_head -> OK. */
  {
    ...
    if (wyl_dl_assert_edb_empty (rows, 1, "policy_violation",
        NULL) != WYRELOG_E_OK)
      return wyl_test_normalize_exit_status (111);
  }
  ...
  /* Case 112: one matching row -> POLICY + witness pointer. */
  {
    ...
    if (wyl_dl_assert_edb_empty (rows, 1, "policy_violation",
      ...
      return wyl_test_normalize_exit_status (112);
    if (wit != &rows[0])
      return wyl_test_normalize_exit_status (1121);
  }
