...
Code that operates on that aggregate must enter
`wyl_handle_lock_engine_session()`. The lock is recursive so one complete
decision, loader, replay, or replacement can retain a single linearization
interval while calling ordinary handle helpers.
...
Raw read/delta getters return borrowed pointers and are reserved for code that
already owns the engine session for the pointer's complete lifetime. New code
should prefer scoped handle helpers. The `engine-session-boundary` test guards
the protected entry-point inventory and rejects production raw-getter callers.
