...
static const wyl_principal_transition_t fsm_table[] = {
  ...
  /* Narrow legacy re-login compatibility.  Issue #752 owns subject-global
   * cross-session precedence and invalidation beyond these two edges. */
  {WYL_PRINCIPAL_STATE_AUTHENTICATED, WYL_PRINCIPAL_EVENT_LOGIN_OK,
      WYL_PRINCIPAL_STATE_MFA_REQUIRED},
  {WYL_PRINCIPAL_STATE_AUTHENTICATED,
