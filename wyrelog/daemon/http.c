...
typedef struct
{
  ...
  gchar *receipt_actor;
  guint8 receipt_fingerprint[WYL_SERVICE_RETIREMENT_FINGERPRINT_BYTES];
...
typedef struct _WylDaemonHttpContext
{
  ...
  * disconnect/shutdown path without depending on response delivery. */
  ...
  WylSession *last_service_publication_session;
  guint service_response_wipe_count;
  gboolean service_response_last_canary_seen;
  gboolean service_response_last_all_zero;
  WylDaemonServiceResponseAuthoritySnapshot service_response_authority;
  WylDaemonServiceResponseCheckpoint service_response_checkpoint;
  gpointer service_response_checkpoint_data;
  WylDaemonServiceResponseRetireCheckpoint service_response_retire_checkpoint;
  gpointer service_response_retire_checkpoint_data;
...
typedef struct
{
  ...
  gchar *text;
  gsize len;
} WylSensitiveServiceTokenResponse;

static void wyl_daemon_http_context_unref (gpointer data);
static WylDaemonHttpContext *wyl_daemon_http_context_ref
  (WylDaemonHttpContext * ctx);
#ifdef WYL_TEST_DAEMON_HTTP
static void service_response_test_checkpoint (WylDaemonHttpContext * ctx,
    gint phase, const WylServiceAuthReservation * reservation);
#endif

static void
wyl_sensitive_service_token_response_free
  (WylSensitiveServiceTokenResponse * response)
{
  if (response == NULL)
    return;
  if (response->text != NULL) {
#ifdef WYL_TEST_DAEMON_HTTP
    static const gchar canary[] = "{\"access_token\":";
    gboolean canary_seen = response->len >= sizeof canary - 1
        && memcmp (response->text, canary, sizeof canary - 1) == 0;
#endif
    sodium_memzero (response->text, response->len);
#ifdef WYL_TEST_DAEMON_HTTP
    gboolean all_zero = TRUE;
    for (gsize i = 0; i < response->len; i++)
      all_zero = all_zero && response->text[i] == '\0';
    if (response->ctx != NULL) {
      WylDaemonHttpContext *ctx = response->ctx;
      g_mutex_lock (&ctx->lock);
      ctx->service_response_wipe_count++;
      ctx->service_response_last_canary_seen = canary_seen;
      ctx->service_response_last_all_zero = all_zero;
      g_mutex_unlock (&ctx->lock);
    }
#endif
    g_free (response->text);
#ifdef WYL_TEST_DAEMON_HTTP
    service_response_test_checkpoint (response->ctx,
        WYL_DAEMON_SERVICE_RESPONSE_BODY_WIPED, NULL);
#endif
  }
  wyl_daemon_http_context_unref (response->ctx);
  sodium_memzero (response, sizeof *response);
  g_free (response);
}

G_DEFINE_AUTOPTR_CLEANUP_FUNC (WylSensitiveServiceTokenResponse,
    wyl_sensitive_service_token_response_free);

typedef enum
{
  WYL_DAEMON_POLICY_WRITE_EMPTY = 0,
  WYL_DAEMON_POLICY_WRITE_ACTIVE,
...
typedef struct
{
  ...
  WylHandle *handle;            /* borrowed while the lease keeps it alive */
  SoupServerMessage *message;   /* borrowed; owns only stack qdata identity */
  wyl_policy_store_t *store;    /* borrowed from the lease-owned pin */
  WylDaemonPolicyWriteState state;
  wyrelog_error_t finalize_rc;
...
#undef WYL_DAEMON_POLICY_WRITE_OWNER_NAME
  ...
  G_STATIC_ASSERT (G_N_ELEMENTS (names)
      == WYL_DAEMON_POLICY_WRITE_OWNER_COUNT);
  return owner >= WYL_DAEMON_POLICY_WRITE_OWNER_KEY_ROTATION
         && owner < WYL_DAEMON_POLICY_WRITE_OWNER_COUNT ? names[owner] : "invalid";
}
...
static void
wyl_daemon_policy_write_detach_message (WylDaemonPolicyWrite *write)
{
  if (write == NULL)
    return;
  if (write->message != NULL
      && g_object_get_data (G_OBJECT (write->message),
      WYL_DAEMON_POLICY_WRITE_DATA) == write)
    g_object_set_data (G_OBJECT (write->message),
        WYL_DAEMON_POLICY_WRITE_DATA, NULL);
  write->message = NULL;
}
...
wyl_daemon_policy_write_clear (WylDaemonPolicyWrite *write)
{
   ...
   * checked finalize path explicitly after unwinding higher-ranked owners.
   * If an invalid live claim prevents consumption, latch the service branch
   * unavailable and deliberately avoid assertion-based process termination. */
  write->store = NULL;
  wyrelog_error_t rc = wyl_service_auth_write_lease_release_terminal
        (&write->lease);
  if (write->lease != NULL) {
    (void) wyl_service_auth_write_lease_terminalize_cleanup (write->lease,
        write->handle);
    rc = wyl_service_auth_write_lease_release_terminal (&write->lease);
  }
  write->handle = NULL;
  write->finalize_rc = rc;
  write->state = WYL_DAEMON_POLICY_WRITE_FINALIZED;
}
...
wyl_daemon_policy_write_finalize (WylDaemonPolicyWrite *write)
{
  if (write == NULL)
    return WYRELOG_E_INVALID;
  if (write->state == WYL_DAEMON_POLICY_WRITE_FINALIZED)
    return write->finalize_rc;
  if (write->state != WYL_DAEMON_POLICY_WRITE_ACTIVE || write->lease == NULL)
    return WYRELOG_E_INVALID;
...
static wyrelog_error_t
wyl_daemon_policy_write_record_primary (WylDaemonPolicyWrite *write,
    wyrelog_error_t primary_rc)
{
  if (write != NULL && !write->primary_rc_recorded) {
    write->primary_rc = primary_rc;
    write->primary_rc_recorded = TRUE;
  }
  return primary_rc;
}
    ...
    wyrelog_error_t primary_rc)
{
  ...
  (void) wyl_daemon_policy_write_finalize (write);
  return primary_rc;
}

#ifdef WYL_TEST_DAEMON_HTTP
static void policy_write_record_non_http_finalize_snapshot
  (WylDaemonPolicyWrite * write, wyrelog_error_t primary_rc,
    wyrelog_error_t cleanup_rc);
#endif

static wyrelog_error_t
wyl_daemon_policy_write_finish_result (WylDaemonPolicyWrite *write,
    wyrelog_error_t primary_rc)
{
  (void) wyl_daemon_policy_write_record_primary (write, primary_rc);
  ...
  if (cleanup_rc != WYRELOG_E_OK)
    WYL_LOG_ERROR (WYL_LOG_SECTION_POLICY,
        "daemon_policy_write_cleanup owner=%s primary_rc=%d primary_rc_recorded=1 primary_status=0 primary_code=non_http cleanup_rc=%d",
        wyl_daemon_policy_write_owner_name (write->owner), primary_rc,
        cleanup_rc);
  return cleanup_rc != WYRELOG_E_OK ? cleanup_rc : primary_rc;
}
...
policy_write_terminal_checkpoint (gpointer data)
{
  WylDaemonHttpContext *ctx = data;
  g_mutex_lock (&ctx->lock);
  ctx->policy_write_terminal_entries++;
  g_mutex_unlock (&ctx->lock);
}
    ...
    wyrelog_error_t primary_rc, wyrelog_error_t cleanup_rc)
{
  ...
  WylDaemonPolicyWriteFinalizeSnapshot snapshot = {
    ...
    .post_finalize_lease_live = write->lease != NULL,
  ...
  g_strlcpy (snapshot.owner_name,
      wyl_daemon_policy_write_owner_name (write->owner),
...
wyl_daemon_policy_write_acquire (WylDaemonHttpContext *ctx,
    SoupServerMessage *message, WylDaemonPolicyWriteOwner owner,
    WylDaemonPolicyWrite *write)
{
  if (ctx == NULL || write == NULL
      || write->state != WYL_DAEMON_POLICY_WRITE_EMPTY
...
#endif
    if (message != NULL)
      g_object_set_data (G_OBJECT (message), WYL_DAEMON_POLICY_WRITE_DATA,
          write);
    ...
    if (rc == WYRELOG_E_OK) {
      ...
      if (fault == WYL_DAEMON_POLICY_WRITE_FINALIZE_FAULT_PREVALIDATION)
        wyl_service_auth_write_lease_test_fail_terminal_prevalidation
          (write->lease);
      else if (fault == WYL_DAEMON_POLICY_WRITE_FINALIZE_FAULT_RANK_AFTER_POP)
        wyl_service_auth_write_lease_test_fail_terminal_rank_after_pop
          (write->lease);
...
#ifdef WYL_TEST_DAEMON_HTTP
static void
service_response_test_checkpoint (WylDaemonHttpContext *ctx, gint phase,
    const WylServiceAuthReservation *reservation)
{
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  WylDaemonServiceResponseCheckpoint checkpoint =
      ctx->service_response_checkpoint;
  gpointer data = ctx->service_response_checkpoint_data;
  g_mutex_unlock (&ctx->lock);
  if (checkpoint != NULL)
    checkpoint (phase, reservation != NULL ? reservation->session_id : NULL,
        reservation != NULL ? reservation->jti : NULL, data);
}
#endif
...
wyl_daemon_http_context_rotate_access_token_key (WylDaemonHttpContext *ctx)
{
  ...
  rc = wyl_daemon_policy_write_acquire (ctx, NULL,
          WYL_DAEMON_POLICY_WRITE_OWNER_KEY_ROTATION, &write);
  if (rc != WYRELOG_E_OK)
    goto out;
    ...
    const gchar *tenant, gboolean create, gboolean sealed)
{
  ...
  if (rc == WYRELOG_E_OK)
    rc = wyl_policy_store_set_tenant_sealed (write.store, tenant, sealed);
  return write.state == WYL_DAEMON_POLICY_WRITE_ACTIVE
      ? wyl_daemon_policy_write_finish_result (&write, rc) : rc;
}
...
void
wyl_daemon_http_service_response_wipe_snapshot_for_test (SoupServer *server,
    guint *out_count, gboolean *out_canary_seen, gboolean *out_all_zero)
{
  if (out_count != NULL)
    *out_count = 0;
  if (out_canary_seen != NULL)
    *out_canary_seen = FALSE;
  if (out_all_zero != NULL)
    *out_all_zero = FALSE;
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL || out_count == NULL || out_canary_seen == NULL
      || out_all_zero == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  *out_count = ctx->service_response_wipe_count;
  *out_canary_seen = ctx->service_response_last_canary_seen;
  *out_all_zero = ctx->service_response_last_all_zero;
  g_mutex_unlock (&ctx->lock);
}

void
wyl_daemon_http_reset_service_response_authority_for_test (SoupServer *server)
{
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  memset (&ctx->service_response_authority, 0,
      sizeof ctx->service_response_authority);
  ctx->service_response_wipe_count = 0;
  ctx->service_response_last_canary_seen = FALSE;
  ctx->service_response_last_all_zero = FALSE;
  g_mutex_unlock (&ctx->lock);
}

void
wyl_daemon_http_set_service_response_checkpoint_for_test (SoupServer *server,
    WylDaemonServiceResponseCheckpoint checkpoint, gpointer data)
{
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  ctx->service_response_checkpoint = checkpoint;
  ctx->service_response_checkpoint_data = data;
  g_mutex_unlock (&ctx->lock);
}
    ...
    WylDaemonServiceResponseRetireCheckpoint checkpoint, gpointer data)
{
  ...
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  ctx->service_response_retire_checkpoint = checkpoint;
  ctx->service_response_retire_checkpoint_data = data;
  g_mutex_unlock (&ctx->lock);
}

void
wyl_daemon_http_set_service_due_write_checkpoint_for_test (SoupServer *server,
    void (*checkpoint) (gpointer data), gpointer data)
{
  ...
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  ctx->service_due_write_checkpoint = checkpoint;
  ctx->service_due_write_checkpoint_data = data;
  g_mutex_unlock (&ctx->lock);
}
    ...
    WylDaemonServiceResponseAuthoritySnapshot * out_snapshot)
{
  ...
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  *out_snapshot = ctx->service_response_authority;
  g_mutex_unlock (&ctx->lock);
}
  ...
  (SoupServer * server, const gchar * session_id)
{
  ...
  WylSession *current = g_hash_table_lookup (ctx->sessions_by_token,
          session_id);
  gboolean matches = current != NULL
      && current == ctx->last_service_publication_session
      && g_strcmp0 (current->service_subject_id, "svc:exchange:mutated") == 0;
  g_mutex_unlock (&ctx->lock);
  return matches;
}
#endif
    ...
    WylDaemonPolicyWriteCheckpoint checkpoint, gpointer data)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, NULL,
          WYL_DAEMON_POLICY_WRITE_OWNER_TEST_POLICY_WRITE, &write);
  if (rc != WYRELOG_E_OK)
    return rc;
  if (checkpoint != NULL)
    checkpoint (data);
  rc = wyl_policy_store_set_tenant_sealed (write.store, WYL_TENANT_DEFAULT,
          FALSE);
  rc = wyl_daemon_policy_write_finish_result (&write, rc);
  return wyl_daemon_policy_write_finalize (&write) == rc
      ? rc : WYRELOG_E_INTERNAL;
}

void
wyl_daemon_http_fail_next_policy_write_finalize_for_test (SoupServer *server,
    WylDaemonPolicyWriteFinalizeFault fault)
{
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL || fault < WYL_DAEMON_POLICY_WRITE_FINALIZE_FAULT_NONE
      || fault > WYL_DAEMON_POLICY_WRITE_FINALIZE_FAULT_PIN_IDENTITY)
    return;
  g_mutex_lock (&ctx->lock);
  ...
  (SoupServer * server)
{
  ...
  if (ctx == NULL)
    return;
  g_mutex_lock (&ctx->lock);
  ctx->fail_next_tenant_seal_verification = TRUE;
  g_mutex_unlock (&ctx->lock);
}
...
guint
wyl_daemon_http_policy_write_terminal_entries_for_test (SoupServer *server)
{
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL)
    return 0;
...
gboolean
wyl_daemon_http_policy_write_finalize_snapshot_for_test (SoupServer *server,
    WylDaemonPolicyWriteFinalizeSnapshot *out_snapshot)
{
  WylDaemonHttpContext *ctx = wyl_daemon_http_get_context (server);
  if (ctx == NULL || out_snapshot == NULL)
...
#ifdef WYL_HAS_AUDIT
static WylSensitiveServiceTokenResponse *
build_service_token_response (WylDaemonHttpContext *ctx,
    const gchar *access_token)
{
  WylSensitiveServiceTokenResponse *response =
      g_try_new0 (WylSensitiveServiceTokenResponse, 1);
  if (response == NULL)
    return NULL;
  response->ctx = wyl_daemon_http_context_ref (ctx);
  g_autoptr (GString) json = g_string_new ("{\"access_token\":");
  append_json_string (json, access_token);
  g_string_append_c (json, '}');
  response->text = g_string_free (g_steal_pointer (&json), FALSE);
  response->len = strlen (response->text);
  return response;
}
#endif
...
static wyrelog_error_t service_token_exchange_core_with_authority
  (WylDaemonHttpContext * ctx,
    const WylDaemonServiceTokenRequest * request, guint * out_status,
    gchar ** out_body, guint * out_retry_after,
    WylSensitiveServiceTokenResponse ** out_sensitive_response,
    WylServiceResponseAuthority ** out_authority);
...
#define WYL_SERVICE_AUTH_RETIRE_BATCH_MAX 32u
...
typedef struct
{
  ...
  gpointer access_key;
  WylAccessTokenState *access_value;
} WylServiceRetiredLivePair;

static void
service_retired_live_pair_free (gpointer data)
{
  WylServiceRetiredLivePair *pair = data;
  if (pair == NULL)
    return;
  g_free (pair->session_key);
  g_clear_object (&pair->session_value);
...
static gchar *
service_response_authority_try_strdup (const gchar *value)
{
  if (value == NULL)
    return NULL;
...
static wyrelog_error_t service_auth_retire_exact (WylDaemonHttpContext * ctx,
    const WylServiceAuthReservation * reservation);
static void service_response_authority_complete
  (WylServiceResponseAuthority * authority,
    WylServiceResponseAuthorityOutcome outcome);

static void
service_response_authority_free (WylServiceResponseAuthority *authority)
{
  if (authority == NULL)
    return;
  WylDaemonHttpContext *ctx = authority->ctx;
  if (g_atomic_int_get (&authority->activated)
      && g_atomic_int_get (&authority->outcome)
      == WYL_SERVICE_RESPONSE_AUTHORITY_PENDING) {
#ifdef WYL_TEST_DAEMON_HTTP
    g_mutex_lock (&ctx->lock);
    ctx->service_response_authority.unclaimed_fallbacks++;
    g_mutex_unlock (&ctx->lock);
    service_response_test_checkpoint (ctx,
        WYL_DAEMON_SERVICE_RESPONSE_UNCLAIMED_FALLBACK,
        &authority->reservation);
#endif
    wyrelog_error_t rc = service_auth_retire_exact (ctx,
            &authority->reservation);
  ...
  if (ctx != NULL) {
    g_mutex_lock (&ctx->lock);
    ctx->service_response_authority.destroyed++;
    g_mutex_unlock (&ctx->lock);
    service_response_test_checkpoint (ctx,
...
#endif
  wyl_sensitive_string_free (authority->reservation.session_id);
  wyl_sensitive_string_free (authority->reservation.jti);
  wyl_sensitive_string_free (authority->reservation.credential_id);
  wyl_sensitive_string_free (authority->reservation.principal);
  wyl_sensitive_string_free (authority->reservation.tenant);
  sodium_memzero (&authority->reservation, sizeof authority->reservation);
  g_clear_object (&authority->handle);
  wyl_daemon_http_context_unref (authority->ctx);
  sodium_memzero (authority, sizeof *authority);
  g_free (authority);
}

G_DEFINE_AUTOPTR_CLEANUP_FUNC (WylServiceResponseAuthority,
    service_response_authority_free);

static WylServiceResponseAuthority *
service_response_authority_new (WylDaemonHttpContext *ctx,
    const WylServiceExchangePublicationView *view)
{
  if (ctx == NULL || view == NULL)
    return NULL;
  g_autoptr (WylServiceResponseAuthority) authority =
      g_try_new0 (WylServiceResponseAuthority, 1);
  if (authority == NULL)
    return NULL;
  authority->ctx = wyl_daemon_http_context_ref (ctx);
  authority->handle = g_object_ref (ctx->handle);
  authority->reservation.session_id = service_response_authority_try_strdup
        (view->session_id);
  authority->reservation.jti =
      service_response_authority_try_strdup (view->jti);
  authority->reservation.credential_id =
      service_response_authority_try_strdup (view->credential_id);
  authority->reservation.principal =
      service_response_authority_try_strdup (view->principal);
  authority->reservation.tenant =
      service_response_authority_try_strdup (view->tenant);
  authority->reservation.generation = view->generation;
  authority->reservation.expires_at = view->expires_at;
  if (authority->reservation.session_id == NULL
      || authority->reservation.jti == NULL
      || authority->reservation.credential_id == NULL
      || authority->reservation.principal == NULL
      || authority->reservation.tenant == NULL)
    return NULL;
#ifdef WYL_TEST_DAEMON_HTTP
  g_mutex_lock (&ctx->lock);
  ctx->service_response_authority.created++;
  ctx->service_response_authority.complete++;
  g_mutex_unlock (&ctx->lock);
#endif
  return g_steal_pointer (&authority);
}

static void
service_response_authority_complete (WylServiceResponseAuthority *authority,
    WylServiceResponseAuthorityOutcome outcome)
{
  if (authority == NULL)
    return;
  ...
  WylDaemonHttpContext *ctx = authority->ctx;
#endif
  if (!g_atomic_int_compare_and_exchange (&authority->outcome,
      WYL_SERVICE_RESPONSE_AUTHORITY_PENDING, outcome)) {
#ifdef WYL_TEST_DAEMON_HTTP
    g_mutex_lock (&ctx->lock);
    ctx->service_response_authority.duplicate_outcomes++;
    g_mutex_unlock (&ctx->lock);
#endif
    return;
  }
  ...
  if (outcome == WYL_SERVICE_RESPONSE_AUTHORITY_FINISHED)
    ctx->service_response_authority.finished++;
  else if (outcome == WYL_SERVICE_RESPONSE_AUTHORITY_ABORTED)
    ctx->service_response_authority.aborted++;
  else
    ctx->service_response_authority.cleanup_failed++;
  g_mutex_unlock (&ctx->lock);
  gint phase = outcome == WYL_SERVICE_RESPONSE_AUTHORITY_FINISHED
      ? WYL_DAEMON_SERVICE_RESPONSE_FINISHED
      : outcome == WYL_SERVICE_RESPONSE_AUTHORITY_ABORTED
      ? WYL_DAEMON_SERVICE_RESPONSE_ABORTED
      : WYL_DAEMON_SERVICE_RESPONSE_CLEANUP_FAILED;
  service_response_test_checkpoint (ctx, phase, &authority->reservation);
#endif
}
    ...
    gpointer user_data)
{
  (void) server;
  (void) user_data;
  WylServiceResponseAuthority *authority = g_object_steal_data (G_OBJECT (msg),
...
static void
service_response_aborted (SoupServer *server, SoupServerMessage *msg,
    gpointer user_data)
{
  (void) server;
  (void) user_data;
...
static gboolean
service_live_state_matches_reservation (WylDaemonHttpContext *ctx,
    const WylServiceAuthReservation *reservation)
{
  WylSession *session = g_hash_table_lookup (ctx->sessions_by_token,
  ...
  return session != NULL && access != NULL
         && wyl_session_get_auth_method_private (session)
         == WYL_SESSION_AUTH_METHOD_SERVICE_CREDENTIAL
         && wyl_session_matches_service_tuple_private (session,
             reservation->session_id, reservation->jti, reservation->principal,
             reservation->tenant, reservation->credential_id,
             reservation->generation,
             wyl_session_get_service_issued_at_seconds_private (session),
             reservation->expires_at)
         && access->auth_method == WYL_SESSION_AUTH_METHOD_SERVICE_CREDENTIAL
         && !access->revoked && g_strcmp0 (access->jti, reservation->jti) == 0
         && g_strcmp0 (access->session_id, reservation->session_id) == 0
         && g_strcmp0 (access->subject, reservation->principal) == 0
...
#ifdef WYL_TEST_DAEMON_HTTP
  g_mutex_lock (&ctx->lock);
  WylDaemonServiceResponseRetireCheckpoint retire_checkpoint =
      ctx->service_response_retire_checkpoint;
  gpointer retire_checkpoint_data =
      ctx->service_response_retire_checkpoint_data;
  g_mutex_unlock (&ctx->lock);
#endif
  WylServiceAuthWriteLease *lease = NULL;
  WylServiceAuthRegistryMaintenanceParticipant *maintenance = NULL;
...
#ifdef WYL_TEST_DAEMON_HTTP
    ctx->last_service_publication_session = candidate->session_value;
#endif
  ...
  if (rc == WYRELOG_E_OK) {
    *out_published_access = candidate->access_value;
    ...
    if (service_publication_fault_is_locked (ctx,
        WYL_DAEMON_SERVICE_PUBLICATION_FAULT_SESSION_ROLLBACK_TUPLE_MUTATION,
        FALSE)) {
      g_free ((*out_published_session)->service_subject_id);
    ...
    WylSession *published_session, WylAccessTokenState *published_access)
{
...
#endif
    session_exact = !mismatch && current == published_session
        && wyl_session_matches_service_tuple_private (current,
            view->session_id, view->jti, view->principal, view->tenant,
            view->credential_id, view->generation, view->issued_at,
            view->expires_at);
  }
  if (published_access != NULL) {
    ...
    gboolean mismatch = FALSE;
#endif
    access_exact = !mismatch && current == published_access
        && service_access_state_matches_view (current, view);
  }
  gboolean exact = session_exact && access_exact;
  if (exact && published_session != NULL
...
/*
 * WRITE -> context -> registry is the only maintenance lock order.  Candidate
 * snapshots are taken before the context lock; the WRITE lease prevents a
 * resolver, exchange ticket, or invalidator from changing the tuple while the
 * exact live companions are prevalidated and removed.
 */
static wyrelog_error_t
service_auth_retire_due (WylDaemonHttpContext *ctx, gint64 now_seconds)
{
  WylServiceAuthWriteLease *lease = NULL;
  GPtrArray *due = NULL;
...
#ifdef WYL_TEST_DAEMON_HTTP
  g_mutex_lock (&ctx->lock);
  void (*write_checkpoint) (gpointer data) = ctx->service_due_write_checkpoint;
  gpointer write_checkpoint_data = ctx->service_due_write_checkpoint_data;
  g_mutex_unlock (&ctx->lock);
#endif
  rc = wyl_service_auth_authority_acquire_write
        (wyl_handle_get_service_auth_authority (ctx->handle), ctx->handle, NULL,
  ...
  if (rc != WYRELOG_E_OK)
    return rc;
#ifdef WYL_TEST_DAEMON_HTTP
  if (write_checkpoint != NULL)
    write_checkpoint (write_checkpoint_data);
#endif
  rc = wyl_service_auth_registry_copy_due (ctx->service_auth_registry,
          now_seconds, WYL_SERVICE_AUTH_RETIRE_BATCH_MAX, &due);
  if (rc != WYRELOG_E_OK)
    goto fail_stop;
  if (due->len == 0)
    goto out;
  ...
  for (guint i = 0; i < due->len; i++) {
    ...
    if (!service_live_state_matches_reservation (ctx, reservation)) {
      rc = WYRELOG_E_POLICY;
  ...
  for (guint i = 0; i < due->len; i++) {
    ...
    g_ptr_array_add (retired, pair);
  }
  rc = service_publication_context_unlock (ctx, WYRELOG_E_OK);
  if (rc != WYRELOG_E_OK)
    goto fail_stop;
  goto out;
...
service_token_exchange_prepare (WylDaemonHttpContext *ctx,
    ...
    gsize credential_secret_len,
    WylSensitiveServiceTokenResponse **out_response,
    WylServiceResponseAuthority **out_authority)
{
  ...
  if (ctx == NULL || credential_id == NULL || credential_secret == NULL
      || credential_secret_len == 0 || out_response == NULL
      || out_authority == NULL)
    return WYRELOG_E_INVALID;
  ...
  rc = wyl_service_exchange_authority_prepare_token (&authority, key_id,
          ...
          now_seconds, token_secret, sizeof token_secret, &prepared);
  sodium_memzero (token_secret, sizeof token_secret);
  if (rc != WYRELOG_E_OK) {
    wyl_service_exchange_authority_clear (&authority);
    return rc;
...
#endif
  if (rc == WYRELOG_E_OK)
    response = build_service_token_response (ctx, view.access_token);
  if (rc == WYRELOG_E_OK && response == NULL)
    rc = WYRELOG_E_NOMEM;
  g_autoptr (WylServiceResponseAuthority) response_authority = NULL;
  if (rc == WYRELOG_E_OK)
    response_authority = service_response_authority_new (ctx, &view);
  if (rc == WYRELOG_E_OK && response_authority == NULL)
    rc = WYRELOG_E_NOMEM;
  if (rc != WYRELOG_E_OK) {
  ...
  if (pre_handoff_failed) {
    service_response_test_checkpoint (ctx,
        ...
        &response_authority->reservation);
...
#endif
  *out_response = g_steal_pointer (&response);
  *out_authority = g_steal_pointer (&response_authority);
  return WYRELOG_E_OK;
}

static wyrelog_error_t
service_token_exchange_core_with_authority (WylDaemonHttpContext *ctx,
    const WylDaemonServiceTokenRequest *request, guint *out_status,
    gchar **out_body, guint *out_retry_after,
    WylSensitiveServiceTokenResponse **out_sensitive_response,
    WylServiceResponseAuthority **out_authority)
{
  ...
  *out_sensitive_response = g_steal_pointer (&response);
  *out_authority = g_steal_pointer (&response_authority);
  *out_status = 200;
  return WYRELOG_E_OK;
}
    ...
    gchar **out_body, guint *out_retry_after)
{
  ...
  wyrelog_error_t rc = service_token_exchange_core_with_authority (ctx, request,
          out_status, out_body, out_retry_after, &response, &authority);
  if (rc == WYRELOG_E_OK && response != NULL) {
    ...
    WylDaemonHttpContext *ctx, const WylDaemonServiceTokenRequest *request)
{
  ...
  g_autoptr (WylServiceResponseAuthority) response_authority = NULL;
  wyrelog_error_t rc = service_token_exchange_core_with_authority (ctx, request,
          &status, &body, &retry_after, &response, &response_authority);
  if (rc != WYRELOG_E_OK) {
  ...
  if (response_authority != NULL) {
#ifdef WYL_TEST_DAEMON_HTTP
    service_response_test_checkpoint (ctx,
        WYL_DAEMON_SERVICE_RESPONSE_PRE_HANDOFF,
        &response_authority->reservation);
    g_mutex_lock (&ctx->lock);
    ctx->service_response_authority.attached++;
    g_mutex_unlock (&ctx->lock);
#endif
    g_object_set_data_full (G_OBJECT (msg),
        WYL_DAEMON_SERVICE_RESPONSE_AUTHORITY_DATA,
        g_steal_pointer (&response_authority),
  ...
  if (response != NULL) {
    ...
    WylSensitiveServiceTokenResponse *body_owner = g_steal_pointer (&response);
    ...
    gsize credential_secret_len, gchar **out_body)
{
  ...
  wyrelog_error_t rc = service_token_exchange_prepare (ctx, credential_id,
          credential_secret, credential_secret_len, &response, &authority);
  if (rc == WYRELOG_E_OK && response != NULL) {
    *out_body = g_steal_pointer (&response->text);
    response->len = 0;
  }
  ...
  if (g_strcmp0 (claims.auth_method, "service_credential") == 0) {
    ...
    if (rc == WYRELOG_E_OK) {
      ...
      if (rc != WYRELOG_E_OK || !found
          ...
          || reservation.expires_at != claims.expires_at)
        rc = WYRELOG_E_POLICY;
    }
    if (rc == WYRELOG_E_OK) {
...
#ifdef WYL_TEST_DAEMON_HTTP
static void
policy_write_count_response_header (const gchar *name, const gchar *value,
    gpointer user_data)
{
  (void) value;
  if (g_ascii_strcasecmp (name, WYL_DAEMON_REQUEST_ID_HEADER) == 0
      || g_ascii_strcasecmp (name, "Content-Type") == 0
      || g_ascii_strcasecmp (name, "Retry-After") == 0) {
...
static void
policy_write_record_finalize_snapshot (WylDaemonPolicyWrite *write,
    SoupServerMessage *msg, guint primary_status, const gchar *primary_code,
    wyrelog_error_t cleanup_rc, guint pre_finalize_status,
    guint pre_finalize_header_count, gsize pre_finalize_body_length)
{
  if (write == NULL || write->test_ctx == NULL)
    return;
  WylDaemonHttpContext *ctx = write->test_ctx;
  ...
  WylDaemonPolicyWriteFinalizeSnapshot snapshot = {
    ...
    .primary_rc = write->primary_rc,
    .primary_rc_recorded = write->primary_rc_recorded,
    .cleanup_rc = cleanup_rc,
    .pre_finalize_status = pre_finalize_status,
    .pre_finalize_header_count = pre_finalize_header_count,
    .pre_finalize_body_length = pre_finalize_body_length,
    .post_finalize_lease_live = write->lease != NULL,
    .post_finalize_store_live = write->store != NULL,
    .post_finalize_total_pins = G_MAXUINT,
    .post_finalize_thread_pins = G_MAXUINT,
  ...
  g_strlcpy (snapshot.primary_code,
      primary_code != NULL ? primary_code : "success",
      sizeof snapshot.primary_code);
  g_strlcpy (snapshot.owner_name,
      wyl_daemon_policy_write_owner_name (write->owner),
      sizeof snapshot.owner_name);
  wyl_handle_policy_store_pin_snapshot_for_test (write->handle != NULL ?
      write->handle : write->test_ctx->handle,
      &snapshot.post_finalize_total_pins, &snapshot.post_finalize_thread_pins);
  WylHandle *handle = write->handle != NULL ? write->handle :
      write->test_ctx->handle;
  snapshot.post_finalize_transaction_active =
...
static wyrelog_error_t
wyl_daemon_policy_write_finalize_for_response (SoupServerMessage *msg,
    guint primary_status, const gchar *primary_code)
{
  WylDaemonPolicyWrite *write = g_object_get_data (G_OBJECT (msg),
          WYL_DAEMON_POLICY_WRITE_DATA);
  if (write == NULL)
    return WYRELOG_E_OK;
#ifdef WYL_TEST_DAEMON_HTTP
  guint pre_finalize_header_count = 0;
  SoupMessageHeaders *response_headers =
      soup_server_message_get_response_headers (msg);
  soup_message_headers_foreach (response_headers,
      policy_write_count_response_header, &pre_finalize_header_count);
  SoupMessageBody *response_body = soup_server_message_get_response_body (msg);
  gsize pre_finalize_body_length =
      response_body != NULL ? response_body->length : 0;
  guint pre_finalize_status = soup_server_message_get_status (msg);
#endif
  wyrelog_error_t cleanup_rc = wyl_daemon_policy_write_finalize (write);
#ifdef WYL_TEST_DAEMON_HTTP
  policy_write_record_finalize_snapshot (write, msg, primary_status,
      primary_code, cleanup_rc, pre_finalize_status,
      pre_finalize_header_count, pre_finalize_body_length);
#endif
  wyl_daemon_policy_write_detach_message (write);
  if (cleanup_rc != WYRELOG_E_OK)
    WYL_LOG_ERROR (WYL_LOG_SECTION_POLICY,
        "daemon_policy_write_cleanup owner=%s primary_rc=%d primary_rc_recorded=%u primary_status=%u primary_code=%s cleanup_rc=%d",
        wyl_daemon_policy_write_owner_name (write->owner), write->primary_rc,
        write->primary_rc_recorded ? 1 : 0,
        primary_status, primary_code != NULL ? primary_code : "success",
        cleanup_rc);
  return cleanup_rc;
}
...
set_json_error (SoupServerMessage *msg, guint status, const gchar *code)
{
  ...
  if (cleanup_rc != WYRELOG_E_OK) {
    status = 500;
    code = "policy_write_cleanup_failed";
  }
  attach_request_id_header (msg);
    ...
    const char *path, GHashTable *query, gpointer user_data)
{
  ...
  } else if (dimension == WYL_POLICY_FACT_QUOTA_SCHEMA_COUNT) {
    rc = wyl_policy_store_get_fact_schema_quota_status
          (ctx->handle != NULL ? wyl_handle_get_policy_store (ctx->handle) : NULL,
  ...
  if (dimension == WYL_POLICY_FACT_QUOTA_WRITE_RATE) {
    ...
    g_string_append_printf (body, ",\"dimension\":\"%s\",\"limit\":", name);
    if (config.has_limit)
...
static gboolean
wyl_daemon_policy_write_prepare_success_response (SoupServerMessage *msg)
{
  if (wyl_daemon_policy_write_finalize_for_response (msg, 200,
      "success") == WYRELOG_E_OK)
    return TRUE;
...
/* These projected audit/state relations use column zero as their unique key.
 * Check all rows for that key: a single full-tuple contains() probe cannot
 * detect an additional row carrying a different value. */
static wyrelog_error_t
verify_tenant_lifecycle_keyed_row (WylEngineVerification *verification,
    const gchar *relation, const gchar *const *symbols, gsize ncols,
    gboolean expected)
{
  ...
  if (!expected) {
    ...
    if (rc != WYRELOG_E_OK)
      return rc;
    gboolean absent = FALSE;
    rc = wyl_engine_verification_has_no_keyed_row (verification, relation,
            row[0], &absent);
    if (rc != WYRELOG_E_OK)
      return rc;
    return absent ? WYRELOG_E_OK : WYRELOG_E_POLICY;
  }
...
static wyrelog_error_t
verify_tenant_lifecycle_input_row (WylEngineVerification *verification,
    const gchar *relation, const gchar *const *symbols, gsize ncols,
    gboolean expected)
{
  g_autofree gint64 *row = g_new0 (gint64, ncols);
    ...
    gpointer data)
{
  ...
  rc = wyl_engine_verification_lookup_symbol (verification,
          publication->audit_id, &event_row[0]);
  if (rc == WYRELOG_E_OK)
  ...
  rc = wyl_engine_verification_has_exact_keyed_row (verification,
          "audit_event", event_row[0], event_row, G_N_ELEMENTS (event_row),
          &exact_event);
  if (rc != WYRELOG_E_OK)
    return rc;
  if (!exact_event)
  ...
  rc = wyl_engine_verification_has_exact_input_row (verification,
          "audit_event_input", event_row, G_N_ELEMENTS (event_row), TRUE,
          &exact_input);
  if (rc != WYRELOG_E_OK || !exact_input)
    return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
  g_autofree gchar *audit_id = wyl_audit_event_dup_id_string
        (publication->audit_event);
  rc = wyl_engine_verification_verify_audit_event (verification, audit_id,
          wyl_audit_event_get_created_at_us (publication->audit_event), TRUE,
          ...
          wyl_audit_event_get_deny_reason (publication->audit_event),
          wyl_audit_event_get_deny_origin (publication->audit_event),
  ...
  for (guint i = 0; i < G_N_ELEMENTS (relations); i++) {
    ...
    g_autofree gchar *input_relation = g_strconcat (relations[i], "_input",
            NULL);
    gint64 input_row[] = { event_row[0], 0 };
    rc = wyl_engine_verification_lookup_symbol (verification, values[i],
            &input_row[1]);
    if (rc == WYRELOG_E_OK)
      rc = wyl_engine_verification_has_exact_input_row (verification,
              input_relation, input_row, G_N_ELEMENTS (input_row), TRUE,
              &exact_input);
    if (rc != WYRELOG_E_OK || !exact_input)
      return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
    rc = verify_tenant_lifecycle_keyed_row (verification, relations[i], row,
            G_N_ELEMENTS (row), TRUE);
    if (rc != WYRELOG_E_OK)
      return rc;
  }
  const gchar *optional_audit_relations[] = {
    "audit_event_deny_reason",
    "audit_event_deny_origin",
  };
  for (guint i = 0; i < G_N_ELEMENTS (optional_audit_relations); i++) {
    gboolean absent = FALSE;
    rc = wyl_engine_verification_has_no_keyed_row (verification,
            optional_audit_relations[i], event_row[0], &absent);
    if (rc != WYRELOG_E_OK || !absent)
      return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
  }
  return WYRELOG_E_OK;
    ...
    gpointer data)
{
  ...
  for (guint i = 0; rc == WYRELOG_E_OK && i < G_N_ELEMENTS (tenant_states); i++) {
    rc = verify_tenant_lifecycle_keyed_row (verification, "session_state",
            tenant_states[i], G_N_ELEMENTS (tenant_states[i]), FALSE);
    if (rc == WYRELOG_E_OK)
      rc = verify_tenant_lifecycle_input_row (verification, "session_state",
              tenant_states[i], G_N_ELEMENTS (tenant_states[i]), FALSE);
  }
  const gchar *membership[] = {
    publication->creator_subject, "wr.system_admin", publication->tenant,
  ...
  if (rc == WYRELOG_E_OK)
    rc = verify_tenant_lifecycle_input_row (verification, "member_of",
  ...
  if (rc == WYRELOG_E_OK)
    rc = wyl_engine_verification_lookup_symbol (verification,
            publication->audit_id, &audit_key);
  /* If this publication's audit id was never interned, no row in any of the
   * keyed audit relations can reference it. That is the expected proof for an
   * absent-create repair, whose contract explicitly removes the audit event. */
  if (rc == WYRELOG_E_NOT_FOUND)
    return WYRELOG_E_OK;
  if (rc != WYRELOG_E_OK)
    return rc;
  const gchar *audit_relations[] = {
    "audit_event",
    "audit_event_subject",
    "audit_event_action",
    "audit_event_resource",
  ...
  const gchar *audit_input_relations[] = {
    "audit_event_input",
  ...
  for (guint i = 0; i < G_N_ELEMENTS (audit_input_relations); i++) {
    gboolean absent = FALSE;
    rc = wyl_engine_verification_has_no_input_key (verification,
            audit_input_relations[i], audit_key, &absent);
    if (rc != WYRELOG_E_OK || !absent)
      return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
  }
  for (guint i = 0; rc == WYRELOG_E_OK
      && i < G_N_ELEMENTS (audit_relations); i++) {
    gboolean absent = FALSE;
    rc = wyl_engine_verification_has_no_keyed_row (verification,
            audit_relations[i], audit_key, &absent);
    if (rc == WYRELOG_E_OK && !absent)
      rc = WYRELOG_E_POLICY;
  }
  return rc;
...
{
  WylDaemonHttpContext *ctx;
  guint64 installed_id;
} WylTenantSealRecoveryLatch;

static WylTenantRecoveryDescriptor *
tenant_seal_recovery_descriptor_new (WylHandle *handle,
    const WylTenantSealPublicationRecovery *recovery)
{
  if (!WYL_IS_HANDLE (handle) || recovery == NULL
      || recovery->receipt_version != WYL_SERVICE_RETIREMENT_RECEIPT_VERSION
      || !wyl_policy_store_tenant_id_is_valid (recovery->tenant_id)
      || !wyl_request_id_is_canonical (recovery->request_id)
      || recovery->actor_subject_id == NULL || recovery->audit_id == NULL
      || recovery->audit_created_at_us <= 0
      || recovery->input_fingerprint == NULL
      || recovery->input_fingerprint_len !=
      WYL_SERVICE_RETIREMENT_FINGERPRINT_BYTES
      || !recovery->invalidation_completed)
    return NULL;
  WylTenantRecoveryDescriptor *descriptor =
      g_new0 (WylTenantRecoveryDescriptor, 1);
  descriptor->ref_count = 1;
  ...
  descriptor->audit_id = g_strdup (recovery->audit_id);
  descriptor->audit_created_at_us = recovery->audit_created_at_us;
  descriptor->audit_subject = g_strdup (recovery->actor_subject_id);
  descriptor->audit_action = g_strdup ("tenant_seal");
  descriptor->audit_resource = g_strdup (recovery->tenant_id);
  descriptor->audit_request_id = g_strdup (recovery->request_id);
  descriptor->audit_decision = WYL_DECISION_ALLOW;
  descriptor->receipt_version = recovery->receipt_version;
  descriptor->receipt_request_id = g_strdup (recovery->request_id);
  descriptor->receipt_actor = g_strdup (recovery->actor_subject_id);
  memcpy (descriptor->receipt_fingerprint, recovery->input_fingerprint,
      sizeof descriptor->receipt_fingerprint);
  descriptor->recorded_lifecycle_generation =
      recovery->tenant_lifecycle_generation;
  descriptor->recorded_sealed_generation = recovery->tenant_sealed_generation;
  descriptor->invalidation_completed = recovery->invalidation_completed;
  if (descriptor->tenant == NULL || descriptor->audit_id == NULL
      || descriptor->audit_subject == NULL || descriptor->audit_action == NULL
      || descriptor->audit_resource == NULL
      || descriptor->audit_request_id == NULL
    ...
    gpointer data)
{
  ...
  gboolean exact_input = FALSE;
  rc = wyl_engine_verification_has_exact_input_row (verification,
          "audit_event_input", event, G_N_ELEMENTS (event), TRUE,
          &exact_input);
  if (rc != WYRELOG_E_OK || !exact_input)
    return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
  rc = wyl_engine_verification_verify_audit_event (verification,
          descriptor->audit_id, descriptor->audit_created_at_us,
          descriptor->audit_decision == WYL_DECISION_ALLOW,
          descriptor->audit_subject, descriptor->audit_action,
          descriptor->audit_resource, descriptor->audit_deny_reason,
          descriptor->audit_deny_origin, descriptor->audit_request_id);
  if (rc != WYRELOG_E_OK)
    return rc;
  ...
  for (guint i = 0; i < G_N_ELEMENTS (relations); i++) {
    ...
    if (values[i] != NULL) {
      ...
      rc = wyl_engine_verification_has_no_input_key (verification,
              input_relation, event[0], &exact_input);
      if (rc == WYRELOG_E_OK && !exact_input)
        rc = WYRELOG_E_POLICY;
      if (rc != WYRELOG_E_OK)
        return rc;
      rc = wyl_engine_verification_has_no_keyed_row (verification, relations[i],
              event[0], &exact);
  ...
  if (descriptor->operation == WYL_TENANT_RECOVERY_SEAL) {
    ...
    wyrelog_error_t proof_rc =
        wyl_policy_store_read_exact_tenant_seal_receipt_proof
          (write->store, descriptor->tenant, descriptor->receipt_actor,
            descriptor->receipt_request_id, descriptor->receipt_version, &proof);
    if (proof_rc == WYRELOG_E_OK
        && (proof.receipt_version != descriptor->receipt_version
        || g_strcmp0 (proof.request_id,
        descriptor->receipt_request_id) != 0
        || g_strcmp0 (proof.tenant_id, descriptor->tenant) != 0
        || g_strcmp0 (proof.actor_subject_id,
        descriptor->receipt_actor) != 0
        || sodium_memcmp (proof.input_fingerprint,
        descriptor->receipt_fingerprint,
        sizeof descriptor->receipt_fingerprint) != 0
        || proof.tenant_lifecycle_generation !=
        descriptor->recorded_lifecycle_generation
        || proof.tenant_sealed_generation !=
        descriptor->recorded_sealed_generation
        || g_strcmp0 (proof.audit_id, descriptor->audit_id) != 0
        || proof.created_at_us != descriptor->audit_created_at_us
    ...
    guint32 receipt_version, const gchar *receipt_request_id)
{
  ...
  gpointer checkpoint_data = NULL;
  g_mutex_lock (&ctx->lock);
  checkpoint = ctx->tenant_recovery_claim_checkpoint;
  checkpoint_data = ctx->tenant_recovery_claim_checkpoint_data;
  ctx->tenant_recovery_claim_checkpoint = NULL;
  ctx->tenant_recovery_claim_checkpoint_data = NULL;
  g_mutex_unlock (&ctx->lock);
  if (checkpoint != NULL)
    checkpoint (checkpoint_data);
#endif
...
static void
set_schema_quota_exceeded_json (SoupServerMessage *msg,
    const WylPolicyFactSchemaQuotaStatus *status)
{
  if (status == NULL || !status->has_limit) {
    set_json_error (msg, 500, "fact_quota_status_failed");
    return;
    ...
    const char *path, GHashTable *query, gpointer user_data)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_SCHEMA_REGISTER, &write);
  if (rc != WYRELOG_E_OK) {
  ...
  gboolean quota_exceeded = FALSE;
  rc = wyl_policy_store_register_fact_relation_schema_with_quota_result
        (write.store, &opts, &quota_exceeded);
  schema_columns_clear (columns, n_columns);
  if (quota_exceeded) {
    WylPolicyFactSchemaQuotaStatus quota_status = { 0 };
    if (wyl_policy_store_get_fact_schema_quota_status (write.store, tenant,
        &quota_status) != WYRELOG_E_OK) {
      set_json_error (msg, 500, "fact_quota_status_failed");
      return;
    }
    set_schema_quota_exceeded_json (msg, &quota_status);
    return;
  }
  if (rc == WYRELOG_E_INVALID || rc == WYRELOG_E_POLICY) {
    set_json_error (msg, 400, "invalid_schema_request");
    ...
    const char *path, GHashTable *query, gpointer user_data)
{
  ...
  if (op == FACT_HTTP_OP_FORGET) {
    ...
    if (rc == WYRELOG_E_OK) {
      /* The delete is durably committed.  Refresh ONLY this graph (issue #546
    ...
    const char *path, GHashTable *query, gpointer user_data, gboolean grant)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_DIRECT_PERMISSION, &write);
  if (rc != WYRELOG_E_OK) {
    ...
    gpointer user_data)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_PERMISSION_TRANSITION, &write);
  if (rc != WYRELOG_E_OK) {
    ...
    const char *path, GHashTable *query, gpointer user_data, gboolean grant)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_ROLE_MEMBERSHIP, &write);
  if (rc != WYRELOG_E_OK) {
    set_policy_mutation_error (msg, rc);
    return;
  }
...
static wyrelog_error_t
verify_mfa_keyed_row (WylEngineVerification *verification,
    const gchar *relation, const gchar *const *symbols, gsize ncols,
    gboolean expected)
{
  if (symbols == NULL || ncols == 0)
...
static wyrelog_error_t
verify_mfa_accepted_input_row (WylEngineVerification *verification,
    const gchar *relation, const gchar *const *symbols, gsize ncols,
    gboolean expected)
{
  g_autofree gint64 *row = g_new0 (gint64, ncols);
    ...
    const gchar *origin, const gchar *request_id)
{
  return wyl_engine_verification_verify_audit_event (verification, id,
             created_at_us, TRUE, subject, action, resource, deny_reason, origin,
             request_id);
}
    ...
    gpointer data)
{
  ...
  wyrelog_error_t rc = verify_mfa_audit_event (verification,
          ...
          mutation->enrollment->id_uuidv7, NULL, mutation->audit_origin,
          mutation->request_id);
  ...
  rc = verify_mfa_audit_event (verification, mutation->revocation_audit_id,
          ...
          NULL, mutation->audit_origin, mutation->request_id);
  if (rc != WYRELOG_E_OK)
    return rc;
  const gchar *permission[] = {
  ...
  rc = verify_mfa_accepted_input_row (verification, "direct_permission",
          permission,
  ...
  rc = verify_mfa_accepted_input_row (verification, "perm_state", state,
          G_N_ELEMENTS (state), TRUE);
  if (rc != WYRELOG_E_OK)
    return rc;
  if (mutation->permission_state_event_id <= 0)
    return WYRELOG_E_POLICY;
  const gchar *state_event_symbols[] = {
    mutation->enrollment->subject_id, "wr.login.skip_mfa", "login", "revoke",
    "armed", "dormant",
  };
  gint64 state_event_row[] = { mutation->permission_state_event_id,
                               0, 0, 0, 0, 0, 0 };
  for (guint i = 0; i < G_N_ELEMENTS (state_event_symbols); i++) {
    rc = wyl_engine_verification_lookup_symbol (verification,
            state_event_symbols[i], &state_event_row[i + 1]);
    if (rc != WYRELOG_E_OK)
      return rc;
  }
  gboolean exact_state_event = FALSE;
  rc = wyl_engine_verification_has_exact_input_row (verification,
          "perm_state_event", state_event_row,
          G_N_ELEMENTS (state_event_row), TRUE, &exact_state_event);
  if (rc != WYRELOG_E_OK || !exact_state_event)
    return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
    ...
    SoupServerMessage *msg, GHashTable *query, WylDaemonHttpContext *ctx)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_OPERATION_RECONCILE, &write);
  if (rc != WYRELOG_E_OK) {
    ...
    SoupServerMessage *msg, GHashTable *query, WylDaemonHttpContext *ctx)
{
  ...
  g_auto (WylDaemonPolicyWrite) write = { 0 };
  wyrelog_error_t rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_OPERATION_RECOVER, &write);
  if (rc != WYRELOG_E_OK) {
    set_json_error (msg, rc == WYRELOG_E_BUSY ? 503 : 500,
        rc == WYRELOG_E_BUSY ?
    ...
    const char *path, GHashTable *query, gpointer user_data)
{
  ...
  rc = wyl_daemon_policy_write_acquire (ctx, msg,
          WYL_DAEMON_POLICY_WRITE_OWNER_MFA_CONFIRM, &write);
  if (rc == WYRELOG_E_OK) {
    g_autoptr (WylEngineSession) engine_session =
        wyl_engine_session_acquire (ctx->handle);
    if (engine_session == NULL)
...
typedef struct
{
  ...
  const gchar *scope;
  gboolean found;
} SelfArmStateEventProbe;

static wyrelog_error_t
self_arm_state_event_probe (gint64 event_id, const gchar *subject,
    const gchar *perm, const gchar *scope, const gchar *event,
    const gchar *from_state, const gchar *to_state, gpointer user_data)
{
  SelfArmStateEventProbe *probe = user_data;
...
verify_self_arm_bundle (WylEngineVerification *verification, gpointer data)
{
  ...
  if (bundle->verify_fail)
    return WYRELOG_E_POLICY;
#endif
  if (bundle->store == NULL)
    return WYRELOG_E_INTERNAL;
  for (guint i = 0; i < G_N_ELEMENTS (bundle->perms); i++) {
    const gchar *perm = bundle->perms[i].perm;
    gboolean exists = FALSE;
    wyrelog_error_t rc = wyl_policy_store_direct_permission_exists
          (bundle->store, bundle->actor, perm, bundle->scope, &exists);
    if (rc != WYRELOG_E_OK)
      return rc;
    if (!exists)
      return WYRELOG_E_POLICY;
    const gchar *direct_symbols[] = { bundle->actor, perm, bundle->scope };
    rc = verify_tenant_lifecycle_input_row (verification,
            "direct_permission", direct_symbols,
            G_N_ELEMENTS (direct_symbols), TRUE);
    if (rc != WYRELOG_E_OK)
      return rc;
    rc = wyl_policy_store_permission_state_is (bundle->store, bundle->actor,
            perm, bundle->scope, "armed", &exists);
    if (rc != WYRELOG_E_OK)
      return rc;
    if (!exists)
      return WYRELOG_E_POLICY;
    if (bundle->perms[i].event_id > 0) {
      SelfArmStateEventProbe probe = {
        .event_id = bundle->perms[i].event_id,
        .subject = bundle->actor,
        .perm = perm,
        .scope = bundle->scope,
      };
      rc = wyl_policy_store_foreach_permission_state_event (bundle->store,
              self_arm_state_event_probe, &probe);
      if (rc != WYRELOG_E_OK)
        return rc;
      if (!probe.found)
        return WYRELOG_E_POLICY;
      gint64 state_event_row[] = { bundle->perms[i].event_id, 0, 0, 0, 0, 0, 0 };
      const gchar *state_event_symbols[] = {
        bundle->actor, perm, bundle->scope, "grant", "dormant", "armed",
      };
      for (guint column = 0; column < G_N_ELEMENTS (state_event_symbols);
          column++) {
        rc = wyl_engine_verification_lookup_symbol (verification,
                state_event_symbols[column], &state_event_row[column + 1]);
        if (rc != WYRELOG_E_OK)
          return rc;
      }
      gboolean exact_state_event = FALSE;
      ...
      if (rc != WYRELOG_E_OK || !exact_state_event)
        return rc == WYRELOG_E_OK ? WYRELOG_E_POLICY : rc;
      rc = verify_mfa_audit_event (verification, bundle->perms[i].audit_id,
              wyl_audit_event_get_created_at_us
                (bundle->perms[i].audit_event), bundle->actor,
              "permission_state.grant", perm, "grant", bundle->scope,
              bundle->request_id);
      if (rc != WYRELOG_E_OK)
        return rc;
    }
    ...
    rc = verify_tenant_lifecycle_input_row (verification, "perm_state",
    ...
    gpointer user_data)
{
  ...
  * logout that wins the race is observed here fail-closed with zero writes. */
  ...
  /* Terminal WRITE-lease release BEFORE the success is staged: a release
   * failure is surfaced fail-closed (500) and no 200 is emitted. */
  if (!wyl_daemon_policy_write_prepare_success_response (msg))
    return;
  set_json_ok (msg);
}
    ...
    WylHandle *handle, WylDaemonRuntime *runtime, GError **error)
{
  ...
  g_signal_connect (server, "request-finished",
      G_CALLBACK (service_response_finished), NULL);
