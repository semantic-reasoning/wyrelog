...
typedef enum
{
  ...
  WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_CONCURRENCY_ARG,
  WYL_DAEMON_STRING_FACT_REPLAY_TENANT_CONCURRENCY_ARG,
  WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_QUEUE_LIMIT_ARG,
  WYL_DAEMON_STRING_FACT_REPLAY_TENANT_QUEUE_LIMIT_ARG,
  WYL_DAEMON_STRING_FACT_REPLAY_ROW_LIMIT_ARG,
  WYL_DAEMON_STRING_FACT_REPLAY_TIME_LIMIT_MS_ARG,
...
options_get_string (const WylDaemonOptions *opts, WylDaemonStringField field)
{
  switch (field) {
    ...
    case WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_CONCURRENCY_ARG:
      return opts->fact_replay_global_concurrency_arg;
    case WYL_DAEMON_STRING_FACT_REPLAY_TENANT_CONCURRENCY_ARG:
      return opts->fact_replay_tenant_concurrency_arg;
    case WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_QUEUE_LIMIT_ARG:
      return opts->fact_replay_global_queue_limit_arg;
    case WYL_DAEMON_STRING_FACT_REPLAY_TENANT_QUEUE_LIMIT_ARG:
      return opts->fact_replay_tenant_queue_limit_arg;
    case WYL_DAEMON_STRING_FACT_REPLAY_ROW_LIMIT_ARG:
      return opts->fact_replay_row_limit_arg;
    case WYL_DAEMON_STRING_FACT_REPLAY_TIME_LIMIT_MS_ARG:
      return opts->fact_replay_time_limit_ms_arg;
    ...
    gchar *value)
{
  switch (field) {
    ...
    case WYL_DAEMON_STRING_EVENT_QUEUE_LIMIT_ARG:
      ...
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_CONCURRENCY_ARG:
      opts->fact_replay_global_concurrency_arg = value;
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_TENANT_CONCURRENCY_ARG:
      opts->fact_replay_tenant_concurrency_arg = value;
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_QUEUE_LIMIT_ARG:
      opts->fact_replay_global_queue_limit_arg = value;
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_TENANT_QUEUE_LIMIT_ARG:
      opts->fact_replay_tenant_queue_limit_arg = value;
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_ROW_LIMIT_ARG:
      opts->fact_replay_row_limit_arg = value;
      return;
    case WYL_DAEMON_STRING_FACT_REPLAY_TIME_LIMIT_MS_ARG:
      opts->fact_replay_time_limit_ms_arg = value;
      return;
    case WYL_DAEMON_STRING_BOOTSTRAP_ADMIN_SUBJECT:
...
static gboolean
parse_uint64_arg (const gchar *value, guint64 min_value, guint64 max_value,
    guint64 *out_value)
{
  if (out_value == NULL || value == NULL || value[0] == '\0'
      || value[0] == '-')
    return FALSE;
  errno = 0;
  gchar *end = NULL;
...
load_config_defaults (WylDaemonOptions *opts, GKeyFile *key_file)
{
  ...
  keyfile_take_string (opts, key_file, "event_queue_limit",
      WYL_DAEMON_STRING_EVENT_QUEUE_LIMIT_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_global_concurrency",
      WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_CONCURRENCY_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_tenant_concurrency",
      WYL_DAEMON_STRING_FACT_REPLAY_TENANT_CONCURRENCY_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_global_queue_limit",
      WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_QUEUE_LIMIT_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_tenant_queue_limit",
      WYL_DAEMON_STRING_FACT_REPLAY_TENANT_QUEUE_LIMIT_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_row_limit",
      WYL_DAEMON_STRING_FACT_REPLAY_ROW_LIMIT_ARG);
  keyfile_take_string (opts, key_file, "fact_replay_time_limit_ms",
      WYL_DAEMON_STRING_FACT_REPLAY_TIME_LIMIT_MS_ARG);
    ...
    gpointer user_data, GError **error)
{
  static const struct
  {
    ...
    {"--fact-replay-global-concurrency",
     WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_CONCURRENCY_ARG},
    {"--fact-replay-tenant-concurrency",
     WYL_DAEMON_STRING_FACT_REPLAY_TENANT_CONCURRENCY_ARG},
    {"--fact-replay-global-queue-limit",
     WYL_DAEMON_STRING_FACT_REPLAY_GLOBAL_QUEUE_LIMIT_ARG},
    {"--fact-replay-tenant-queue-limit",
     WYL_DAEMON_STRING_FACT_REPLAY_TENANT_QUEUE_LIMIT_ARG},
    {"--fact-replay-row-limit",
     WYL_DAEMON_STRING_FACT_REPLAY_ROW_LIMIT_ARG},
    {"--fact-replay-time-limit-ms",
     WYL_DAEMON_STRING_FACT_REPLAY_TIME_LIMIT_MS_ARG},
    ...
    GError **error)
{
  GOptionEntry entries[] = {
    ...
    {"fact-replay-global-concurrency", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Maximum concurrent graph replays", "N"},
    {"fact-replay-tenant-concurrency", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Maximum concurrent graph replays per tenant",
     "N"},
    {"fact-replay-global-queue-limit", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Maximum pending graph replays", "N"},
    {"fact-replay-tenant-queue-limit", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Maximum pending graph replays per tenant",
     "N"},
    {"fact-replay-row-limit", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Maximum source rows per graph replay", "N"},
    {"fact-replay-time-limit-ms", 0, 0, G_OPTION_ARG_CALLBACK,
     parse_string_option, "Graph replay publication deadline", "MS"},
...
wyl_daemon_options_resolve (WylDaemonOptions *opts, GError **error)
{
  ...
  wyl_fact_replay_scheduler_config_defaults (&opts->fact_replay_scheduler);
  struct
  {
    const gchar *value;
    guint *target;
    ...
    { opts->fact_replay_global_concurrency_arg,
      &opts->fact_replay_scheduler.global_concurrency,
      "fact replay global concurrency" },
    { opts->fact_replay_tenant_concurrency_arg,
      &opts->fact_replay_scheduler.tenant_concurrency,
      "fact replay tenant concurrency" },
    { opts->fact_replay_global_queue_limit_arg,
      &opts->fact_replay_scheduler.global_queue_limit,
      "fact replay global queue limit" },
    { opts->fact_replay_tenant_queue_limit_arg,
      &opts->fact_replay_scheduler.tenant_queue_limit,
      "fact replay tenant queue limit" },
  };
  for (guint i = 0; i < G_N_ELEMENTS (replay_uints); i++) {
    if (replay_uints[i].value != NULL
        && !parse_uint_arg (replay_uints[i].value, 1, G_MAXUINT,
        replay_uints[i].target)) {
      g_set_error (error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
          "%s must be a positive integer", replay_uints[i].name);
      return FALSE;
    }
  }
  if (opts->fact_replay_row_limit_arg != NULL
      && !parse_uint64_arg (opts->fact_replay_row_limit_arg, 1, G_MAXUINT64,
      &opts->fact_replay_scheduler.row_limit)) {
    g_set_error (error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
        "fact replay row limit must be a positive integer");
    return FALSE;
  }
  if (opts->fact_replay_time_limit_ms_arg != NULL) {
    guint64 milliseconds = 0;
    if (!parse_uint64_arg (opts->fact_replay_time_limit_ms_arg, 1,
        (guint64) G_MAXINT64 / 1000u, &milliseconds)) {
      g_set_error (error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
          "fact replay time limit must be a positive millisecond value");
      return FALSE;
    }
    opts->fact_replay_scheduler.time_limit_us = (gint64) milliseconds * 1000;
  }
  if (wyl_fact_replay_scheduler_config_validate
        (&opts->fact_replay_scheduler) != WYRELOG_E_OK) {
    g_set_error (error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
        "fact replay limits must reserve at least one worker and queue slot "
        "outside each tenant");
    return FALSE;
  }
