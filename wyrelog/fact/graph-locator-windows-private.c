...
operation_evidence_is_valid (const WylFactGraphWinOperationEvidence *evidence)
{
  ...
  return evidence != NULL
         ...
         && (evidence->operation_uuid[6] & 0xf0) == 0x70
         && (evidence->operation_uuid[8] & 0xc0) == 0x80
...
static gboolean
operation_evidence_matches_id (const WylFactGraphWinOperationEvidence *evidence,
    const wyl_id_t *operation_id)
{
  return operation_evidence_is_valid (evidence) && operation_id != NULL
         && memcmp (evidence->operation_uuid, operation_id->bytes,
             sizeof evidence->operation_uuid) == 0;
}
    ...
    const WylFactGraphWinIdentity *artifact_identity)
{
  ...
  memcpy (out_evidence->operation_uuid, operation_id->bytes,
      sizeof out_evidence->operation_uuid);
    ...
    gboolean validate_acl)
{
  ...
  if (rc == WYRELOG_E_OK && validate_acl)
    rc = wyl_fact_graph_win_validate_owner_only_acl (handle);
  return rc;
}
    ...
    WylFactGraphWinIdentity *out_identity)
{
  ...
  WylFactGraphWinOwnerOnlySecurity security;
  ...
  *out_handle = INVALID_HANDLE_VALUE;
  ...
  if (create) {
    rc = wyl_fact_graph_win_owner_only_security_init (&security,
    ...
    WylFactGraphWinIdentity *out_identity, gboolean validate_final_acl)
{
  ...
  *out_handle = INVALID_HANDLE_VALUE;
  ...
  if (rc == WYRELOG_E_OK && validate_final_acl)
    rc = wyl_fact_graph_win_validate_owner_only_acl (current);
  if (rc != WYRELOG_E_OK) {
    if (handle_is_valid (current))
      CloseHandle (current);
    return rc;
    ...
    const WylFactGraphWinIdentity *expected, gboolean strict_acl)
{
  ...
  if (rc == WYRELOG_E_OK && strict_acl)
    rc = wyl_fact_graph_win_validate_protected_owner_acl (handle, 0);
  return rc;
}
    ...
    HANDLE *out_handle, WylFactGraphWinIdentity *out_identity)
{
  ...
  *out_handle = INVALID_HANDLE_VALUE;
  ...
  memset (&security, 0, sizeof security);
  if (create) {
    rc = wyl_fact_graph_win_owner_only_security_init (&security, 0);
    if (rc != WYRELOG_E_OK)
      return rc;
  }
  name.Length = (USHORT) (units * sizeof (WCHAR));
    ...
    WylFactGraphWinIdentity *out_identity)
{
  ...
  *out_handle = INVALID_HANDLE_VALUE;
  memset (out_identity, 0, sizeof *out_identity);
  memset (&security, 0, sizeof security);
  wyrelog_error_t rc = wyl_fact_graph_win_owner_only_security_init (&security,
          0);
  if (rc != WYRELOG_E_OK)
    return rc;
    ...
    const gchar *basename)
{
  ...
  HANDLE handle = INVALID_HANDLE_VALUE;
  WylFactGraphWinIdentity identity = { 0 };
  WylFactGraphWinTokenIdentity token_identity;
  memset (&token_identity, 0, sizeof token_identity);
  if (rc == WYRELOG_E_OK)
    rc = wyl_fact_graph_win_token_identity_init (&token_identity);
  if (rc == WYRELOG_E_OK)
    rc = open_relative_regular (directory->graph_handle, basename,
            GENERIC_READ | GENERIC_WRITE | WRITE_DAC | WRITE_OWNER, FALSE, FALSE,
            &handle, &identity);
  if (rc == WYRELOG_E_OK)
    rc = wyl_fact_graph_win_validate_upgradeable_file_acl (handle,
            &token_identity);
  if (rc == WYRELOG_E_OK)
  ...
  if (handle_is_valid (handle))
    CloseHandle (handle);
  wyl_fact_graph_win_token_identity_clear (&token_identity);
  return rc;
}

static wyrelog_error_t
operation_uuid_parse_canonical (const gchar *operation_uuid,
    wyl_id_t *out_operation_id)
{
  if (operation_uuid == NULL || out_operation_id == NULL)
    return WYRELOG_E_INVALID;
  gchar canonical[WYL_ID_STRING_BUF];
  if (wyl_id_parse (operation_uuid, out_operation_id) != WYRELOG_E_OK
      || wyl_id_format (out_operation_id, canonical, sizeof canonical)
      != WYRELOG_E_OK || g_strcmp0 (operation_uuid, canonical) != 0)
    return WYRELOG_E_INVALID;
  return WYRELOG_E_OK;
}
    ...
    gchar **out_stage_basename)
{
  ...
  wyl_id_t id;
  gchar canonical[WYL_ID_STRING_BUF];
  wyrelog_error_t rc = operation_uuid_parse_canonical (operation_uuid, &id);
  if (rc != WYRELOG_E_OK)
    return rc;
  if (wyl_id_format (&id, canonical, sizeof canonical) != WYRELOG_E_OK)
...
exact_stage_populate (WylFactGraphDirectory *directory, HANDLE handle,
    ...
    const wyl_id_t *operation_id, WylFactGraphStage *out_stage)
{
  ...
  operation_evidence_init (&out_stage->operation_evidence,
      operation_id, &directory->graph_identity, identity);
    ...
    WylFactGraphStage * out_stage)
{
  ...
  if (rc == WYRELOG_E_OK && !identity_equal (&directory->graph_identity,
    ...
    rc = WYRELOG_E_POLICY;
  if (rc == WYRELOG_E_OK && !operation_evidence_matches_id (expected_evidence,
      &operation_id))
    rc = WYRELOG_E_POLICY;
  HANDLE handle = INVALID_HANDLE_VALUE;
  WylFactGraphWinIdentity identity = { 0 };
...
stage_publish_bound (WylFactGraphDirectory *directory, WylFactGraphStage *stage)
{
  ...
  if (rc == WYRELOG_E_OK)
    rc = directory_revalidate (directory);
  if (rc == WYRELOG_E_OK)
    rc = revalidate_named_regular (directory, stage->final_basename,
            borrow_stage_handle (stage), &stage->identity, TRUE);
  if (rc == WYRELOG_E_OK)
    rc = directory_revalidate (directory);
  if (rc == WYRELOG_E_OK)
