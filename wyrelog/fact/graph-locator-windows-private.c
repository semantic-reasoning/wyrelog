...
static wyrelog_error_t
owner_only_security_init (WylOwnerOnlySecurity *security)
{
  wyrelog_error_t rc;
  DWORD sid_length;
  DWORD acl_length;
...
/* File CRT descriptors and durable publication are implemented by the next
 * atomic unit.  Keep every mutation fail-closed until that handle/descriptor
 * bridge has its own adversarial coverage. */
wyrelog_error_t
    ...
    const gchar *basename, gboolean writable, gint *out_fd)
{
  ...
  if (out_fd != NULL)
    *out_fd = -1;
  return out_fd == NULL ? WYRELOG_E_INVALID : WYRELOG_E_POLICY;
}
