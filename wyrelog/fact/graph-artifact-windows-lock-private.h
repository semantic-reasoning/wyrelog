 ...
 * directory identity.  On success |lock_handle| is consumed and retained only
 * as a private guardian duplicate; it is never adapted to a CRT descriptor
 * or reopened by path. */
 ...
 * the lease retains a private duplicate. Reader leases share; mutation leases
 * are exclusive. All contention is non-blocking and returns BUSY. */
wyrelog_error_t
