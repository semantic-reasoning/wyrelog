    ...
    gchar **argv)
{
  ...
  GOptionEntry entries[] = {
    ...
    {"schema-version", 0, 0, G_OPTION_ARG_STRING, &opts.schema_version_arg,
     "Positive version for the relation's first schema registration; later "
     "schema evolution is not supported", "N"},
    ...
    gint argc, gchar **argv)
{
  ...
  if (g_strcmp0 (dimension, "logical_bytes") == 0) {
    ...
    if (opts.rate_per_second_arg != NULL || opts.burst_arg != NULL
        || (configure && opts.limit_arg == NULL)) {
      g_printerr ("wyctl: %s requires --limit and rejects rate options\n",
          g_strcmp0 (dimension, "concurrent_opens") == 0
  ...
  } else if (g_strcmp0 (dimension, "concurrent_opens") == 0) {
    rc = configure
        ? wyl_client_fact_concurrent_open_quota_configure (client, tenant,
            (guint64) parsed_limit, guard_timestamp, opts.guard_loc_class,
            guard_risk, &concurrent_status)
  ...
  } else if (exit_rc == 0 && g_strcmp0 (dimension, "concurrent_opens") == 0) {
    g_print ("tenant=%s dimension=concurrent_opens limit=",
        concurrent_status.tenant_id);
    if (concurrent_status.has_limit)
      g_print ("%" G_GUINT64_FORMAT, concurrent_status.hard_limit);
    else
      g_print ("unlimited");
    g_print (" pending=%" G_GUINT64_FORMAT " active=%" G_GUINT64_FORMAT
        " acquiring=%" G_GUINT64_FORMAT " cleanup_pending=%"
        G_GUINT64_FORMAT " charged=%" G_GUINT64_FORMAT "\n",
    ...
    gchar **argv)
{
  ...
  g_autoptr (GError) error = NULL;
  g_autoptr (GOptionContext) context =
      g_option_context_new ("- wyrelog service-credential rotate");
  g_option_context_add_main_entries (context, entries, NULL);
  if (!g_option_context_parse (context, &argc, &argv, &error)) {
    g_printerr ("wyctl: %s\n", error->message);
    return 2;
  ...
  if (argc > 1) {
    g_printerr ("wyctl: unexpected service-credential rotate argument: %s\n",
        argv[1]);
    return 2;
  }
...
/* Print one non-secret service credential row as a single key=value line.
 * String DTO fields (credential_id / subject_id / state / rotated_from_id)
 * can be NULL, so guard each %s with a "-" placeholder; the numeric fields
 * are always present. generation is guint64; the *_at_us timestamps are
 * gint64. No secret material is ever carried by this DTO. */
static void
print_service_credential_row (const WylClientServiceCredential *credential)
{
  const gchar *credential_id =
      credential->credential_id != NULL ? credential->credential_id : "-";
  const gchar *subject_id =
      credential->subject_id != NULL ? credential->subject_id : "-";
  const gchar *state = credential->state != NULL ? credential->state : "-";
  const gchar *rotated_from_id =
      credential->rotated_from_id != NULL ? credential->rotated_from_id : "-";

  g_print ("credential_id=%s subject_id=%s generation=%" G_GUINT64_FORMAT
      " state=%s created_at_us=%" G_GINT64_FORMAT " expires_at_us=%"
      G_GINT64_FORMAT " revoked_at_us=%" G_GINT64_FORMAT
      " rotated_from_id=%s\n", credential_id, subject_id,
      credential->generation, state, credential->created_at_us,
      credential->expires_at_us, credential->revoked_at_us, rotated_from_id);
}
    ...
    gchar **argv)
{
  ...
  g_autoptr (GError) error = NULL;
  g_autoptr (GOptionContext) context =
      g_option_context_new ("- wyrelog service-credential list");
  g_option_context_add_main_entries (context, entries, NULL);
  if (!g_option_context_parse (context, &argc, &argv, &error)) {
    g_printerr ("wyctl: %s\n", error->message);
    return 2;
  ...
  if (argc > 1) {
    g_printerr ("wyctl: unexpected service-credential list argument: %s\n",
        argv[1]);
    return 2;
  }
...
/* Display map over the #475 client reconcile-operation enum. This is a
...
/* Print one non-secret durable service-credential operation record as a single
 * key=value line. Every field is drawn from the daemon's strict non-secret
 * allow-list, so no secret warning is needed. String fields may be NULL and
 * are guarded with "-"; operation is an enum rendered via the display map and
    ...
    gboolean with_recovery)
{
  ...
  const gchar *state = e->state != NULL ? e->state : "-";
  const gchar *destination = e->destination != NULL ? e->destination : "-";
  const gchar *successor_credential_id =
      e->successor_credential_id != NULL ? e->successor_credential_id : "-";

  g_print ("request_id=%s operation=%s state=%s destination=%s"
      " successor_credential_id=%s expected_generation=%" G_GUINT64_FORMAT
      " successor_generation=%" G_GUINT64_FORMAT " created_at_us=%"
      G_GINT64_FORMAT " updated_at_us=%" G_GINT64_FORMAT " expires_at_us=%"
      G_GINT64_FORMAT, request_id,
      service_credential_operation_string (e->operation), state, destination,
      successor_credential_id, e->expected_generation, e->successor_generation,
      e->created_at_us, e->updated_at_us, e->expires_at_us);
  if (with_recovery) {
    const gchar *recovery = e->recovery != NULL ? e->recovery : "-";
    g_print (" recovery=%s", recovery);
  }
  g_print ("\n");
}
    ...
    gchar **argv)
{
  ...
  g_autoptr (GError) error = NULL;
  g_autoptr (GOptionContext) context =
      g_option_context_new ("- wyrelog service-credential recover");
  g_option_context_add_main_entries (context, entries, NULL);
  if (!g_option_context_parse (context, &argc, &argv, &error)) {
    g_printerr ("wyctl: %s\n", error->message);
    return 2;
  ...
  if (argc > 1) {
    g_printerr ("wyctl: unexpected service-credential recover argument: %s\n",
        argv[1]);
    return 2;
  }
    ...
    gchar **argv)
{
  ...
  g_autoptr (GError) error = NULL;
  g_autoptr (GOptionContext) context =
      g_option_context_new ("- wyrelog service-credential status");
  g_option_context_add_main_entries (context, entries, NULL);
  if (!g_option_context_parse (context, &argc, &argv, &error)) {
    g_printerr ("wyctl: %s\n", error->message);
    return 2;
  ...
  if (argc > 1) {
    g_printerr ("wyctl: unexpected service-credential status argument: %s\n",
        argv[1]);
    return 2;
  }
    ...
    gchar **argv)
{
  ...
  if (g_strcmp0 (argv[1], "recover") == 0)
    return run_service_credential_recover (global_opts, argc - 1, argv + 1);
  if (g_strcmp0 (argv[1], "status") == 0)
    return run_service_credential_status (global_opts, argc - 1, argv + 1);
