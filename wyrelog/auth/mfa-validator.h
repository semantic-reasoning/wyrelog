 ...
 * driven through wyl_session_mfa_verify_with_publishing_validator: this
 * validator publishes the principal FSM transition itself, atomically
 * with consuming the proof (issue #751), rather than leaving it to the
 * boundary.  The 6-digit TOTP proof is accepted against the per-subject
 * enrollment stored in the policy authority store.
 *
