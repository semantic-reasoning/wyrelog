...
wyl_policy_store_create_schema (wyl_policy_store_t *store)
{
  ...
  static const gchar *ddl =
      ...
      "  expected_format_version INTEGER NOT NULL CHECK "
      "    (expected_format_version > 0),"
      "  expected_path_encoding_version INTEGER NOT NULL CHECK "
      "    (expected_path_encoding_version > 0),"
      ...
      "     AND substr(canonical_relative_path, 1, 1) <> '/' "
      ...
      "CREATE UNIQUE INDEX IF NOT EXISTS idx_fact_reconcile_journal_active "
      "  ON fact_reconcile_journal (tenant_id, graph_id) WHERE state NOT IN "
      "    ('done', 'aborted', 'needs_review');"
